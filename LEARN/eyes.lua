return {
    drawEyes = function(eyeX, eyeY)
        local distanceX = love.mouse.getX() - eyeX
        local distanceY = love.mouse.getY() - eyeY

        local distance = math.min(math.sqrt(distanceY ^ 2 + distanceX ^ 2), 30)
        local angle = 2 * math.atan((math.sqrt(distanceX ^ 2 + distanceY ^ 2) - distanceX) / distanceY)
